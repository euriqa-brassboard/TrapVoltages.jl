...
# This should be pointing to the RS1096_12400_*.bin files (without suffix or extension).
const potential_prefix = ARGS[1]
const data_prefix = joinpath(@__DIR__, "../data/hoa/rf_trap")
...
h5open("$(data_prefix).h5", "w") do fh
    for (i, suffix) in enumerate(("1", "L1", "L2", "L3", "L4"))
        g = create_group(fh, "$i")
        xs_um, rf_y2s, rf_yzs, rf_z2s = compute_trap("$(potential_prefix)_$(suffix).bin",
                                                     "hoa", i)
        g["xs_um"] = xs_um
